...
public class Rotation extends SubsystemBase {
  ...
  private Rotation2d rotationRadiansFieldOriented = new Rotation2d();
    ...
    public Rotation2d wantedRotationRadiansBotOriented; // the angle of the barrel relative to the front of the robot
    public Rotation2d wantedRotationRadiansFieldOriented;
    public Pose2d targetLockPose;
   ...
   */
    public systemState handleStateTransitions() {
      if (WantedState != wantedRotationState.ROBOT_ORIENTED_ANGLE
        ...
        state = new State(inputs.rotationPositionRad, inputs.rotationVelocityRadPerSec); // if we are not currently tracking a trapezoidal profile with the current state, set it to follow the bots actual motion to be ready for beginning the profile
      }
    ...
    public void applyStates() {
      switch (SystemState) {
        ...
        case BOT_ADJUSTED_ROBOT_ORIENTED_ANGLE:
          runRobotAdjustedAngle(); // does math to eliminate the bot rotation, then sets the motor while accounting for bot rotational velocity
          break;
        case FIELD_ORIENTED_ANGLE:
          runFieldOrientatedAngle(); // converts to field orientated angle to robot orientated, then calls robotAdjusted to finish
          break;
        case TARGET_LOCK:
          runFieldOrientatedAngle(); // finds angle to target, then calls runFieldOrientedAngle to finish
          break;
        case IDLE:
          break;
      }
    ...
    public void runRobotAdjustedAngle() {
      ...
      goal = new State(wantedRotationRadiansBotOriented.getRadians() - robotOrientation.getRadians(), 0);
    ...
    /** sets the optomizes our wanted angle, then creates a motion profile and feedforward to go to it 
     * 
     * @param veloCompensation if the final velocity given to feed forward should compensate for the robots yaw velocity
    */
    ...
    /** turns the field oriented angle into a robot oriented one, wrapping to avoid going beyond 0 and 2pi, then call runNonAdjustedAngle to set rotation to it */
    public void runFieldOrientatedAngle(){
      
      //convert angle
      wantedRotationRadiansBotOriented = Rotation2d.fromRadians(wrapRadians((wantedRotationRadiansFieldOriented.minus(robotOrientation)).getRadians()));
      // goal = new State(optimizeAngle(wantedRotationRadiansBotOriented).getRadians(), 0.0); shouldn't be necesary
      
      // goTo the angle while compensating for bot movement
      runNonAdjustedAngle(true);
    }

    /** finds the field oriented angle to the target position and sets it as goal, then calls runFieldOrientedAngle to handle all logic from theere */
    public void runTargetLock(){
      double targetFieldAngle = Math.atan2(targetLockPose.getY() - robotPose.getY(), targetLockPose.getX() - robotPose.getX());
      wantedRotationRadiansFieldOriented = Rotation2d.fromRadians(targetFieldAngle);
      runFieldOrientatedAngle();
    }
  
    /**
   * sets the rotation's wanted state 
   * should be the primary way of manipulating the rotation outside of the class
   * 
   ...
   */
    ...
    /**
     * Minimize the rotation required to get to a point
     * If this is used with the PIDController class's continuous input
     * functionality, the furthest a wheel will ever rotate is 90 degrees.
     *  
     * @param desired the rotation we wish to optomize
     * @return a rotation 2d set to be as close as possible to the other while maintaining the previous rotational status
     */
    ...
    /** used for initiating our turrets field orientated rotation by setting it as the same as our bot + any pre determined starting turn */
