...
public class Elevation extends SubsystemBase {
  private final ElevationIO io; // the interface used by elevation, will be defined as a ElevationIOSpark if real

  private final ElevationIOInputsAutoLogged inputs = new ElevationIOInputsAutoLogged(); // logged inputs of the elevation
  ...
  public enum wantedElevationState {
    ...
    MANUAL, // when this is called it means the motor begins moving, so using manual is more like turn this state on and off
    GOTO_ANGLE,
    IDLE
  }
  ...
  public systemState handlStateTransitions() {
    if (SystemState != systemState.GOTO_ANGLE) {
      state = new State(inputs.elevationPositionRad, inputs.elevationVelocityRadPerSec); // if we are not currently tracking a trapezoidal profile with the current state, set it to follow the bots actual motion to be ready for beginning the profile
    }
  ...
  /**
   * sets the elevation's wanted state 
   * should be the primary way of manipulating the elevation outside of the class
   * 
